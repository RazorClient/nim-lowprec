...
for i in 0 ..< K: xf[i] = float32(i mod 7) * 0.1'f32 - 0.3'f32
...
for r in 0 ..< M:
  for c in 0 ..< K:
    ...
    if (c and 1) == 0: w4[bi] = n
    else:              w4[bi] = w4[bi] or (n shl 4)
...
block: # ---- SDOT over PACKED int4 weights: half the bytes per weight ----
  var g = flopGroup("int4 weights (packed), int8 activations (SDOT)", shape, flops, tol = 0.0)
  g.measure "serial", y[mid]:
    quantize(xf, xp, xq)
