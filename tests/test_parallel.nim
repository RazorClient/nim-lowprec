...
template checkExact(serialCall, parCall: untyped) =
  ...
  for nt {.inject.} in threadCounts:
    for i in 0 ..< M: y[i] = 0.0'f32
    parCall
    var mism = 0
    for i in 0 ..< M:
      if cast[uint32](y[i]) != cast[uint32](want[i]): inc mism
    check mism == 0
...
suite "parallel GEMV == serial GEMV, bit-for-bit":
  ...
  test "int8 weights":
    for M in rowCounts:
      ...
      for i in 0 ..< scales.len: scales[i] = 0.02'f32 + float32(i mod 3) * 0.001'f32
      checkExact(dequantGemv(wq, scales, gs, x, y),
  ...
  test "packed int4 weights":
    for M in rowCounts:
      ...
      var scales = newSeq[float32](M * gpr)
      for i in 0 ..< w4.len: w4[i] = uint8((i * 7 + 3) and 0xff)
      for i in 0 ..< scales.len: scales[i] = 0.02'f32 + float32(i mod 3) * 0.001'f32
      checkExact(dequantGemvI4(w4, scales, gs, x, y),
                 parDequantGemvI4(w4, scales, gs, x, y, nt))

  test "packed MXFP4 weights + E8M0 scales":
    for M in rowCounts:
      var wf4 = newSeq[byte](M * (K div 2))
      var scales = newSeq[float32](M * gpr)
      for i in 0 ..< wf4.len: wf4[i] = uint8((i * 11 + 5) and 0xff)
      for i in 0 ..< scales.len: scales[i] = toFloat32(toE8M0(0.03125'f32))
      checkExact(dequantGemvF4(wf4, scales, gs, x, y),
  ...
  test "ggml Q8_0 blocks":
    ...
    for M in rowCounts:
      ...
      for b in 0 ..< w.len:
        ...
        for i in 0 ..< QK: w[b].qs[i] = int8(((b + i) mod 255) - 127)
      checkExact(dequantGemvQ8_0(w, bpr, x, y),
                 parDequantGemvQ8_0(w, bpr, x, y, nt))
  ...
  test "ggml Q4_0 blocks":
    ...
    for M in rowCounts:
      ...
      checkExact(dequantGemvQ4_0(w, bpr, x, y),
    ...
    test "pooled int8-activation kernels, reused across calls":
      ...
      try:
        for M in [300, 129, 64]:
          ...
          for i in 0 ..< wq.len: wq[i] = I8(int8((i mod 200) - 100))
          for i in 0 ..< w4.len: w4[i] = uint8((i * 13 + 7) and 0xff)
          for i in 0 ..< wScales.len: wScales[i] = 0.019'f32 + float32(i mod 4) * 0.0007'f32
          for i in 0 ..< K: xq[i] = I8(int8((i mod 150) - 75))
          for i in 0 ..< gpr: xScales[i] = 0.027'f32 + float32(i mod 3) * 0.0011'f32
    ...
    test "int8 and packed int4 multi-column, reused pool":
      ...
      try:
        for M in [300, 128]:
          ...
          for i in 0 ..< w4.len: w4[i] = uint8((i * 13 + 7) and 0xff)
          for i in 0 ..< wScales.len: wScales[i] = 0.019'f32 + float32(i mod 4) * 0.0007'f32
          for i in 0 ..< xq.len: xq[i] = I8(int8((i mod 150) - 75))
          for i in 0 ..< xScales.len: xScales[i] = 0.027'f32 + float32(i mod 3) * 0.0011'f32
          ...
          for _ in 1 .. 2:
            for i in 0 ..< got.len: got[i] = 0.0'f32
