...
template mxFloatSuite(TT, toFn: untyped; nm, decf, encf: static string; ncodes: static int) =
  suite nm:
    ...
    test "decode all codes vs ml_dtypes":
      ...
      if not fileExists(p):
        ...
        for c in 0 ..< ncodes:
          ...
          if cast[uint32](TT(uint8(c)).toFloat32) != want: inc mm
        check mm == 0
    ...
    test "encode f32 vs ml_dtypes (finite edges + 2M random; NaN skipped)":
      ...
      if not fileExists(p):
        ...
        for i in 0 ..< n:
          ...
          if inF != inF: continue                    # NaN input undefined for finite formats
          if bits(toFn(inF)) != want: inc mm
        check mm == 0

mxFloatSuite(F4E2M1, toF4E2M1, "MXFP4 e2m1", "ref_f4e2m1_to_f32.bin", "ref_f32_to_f4e2m1.bin", 16)
mxFloatSuite(F6E2M3, toF6E2M3, "MXFP6 e2m3", "ref_f6e2m3_to_f32.bin", "ref_f32_to_f6e2m3.bin", 64)
mxFloatSuite(F6E3M2, toF6E3M2, "MXFP6 e3m2", "ref_f6e3m2_to_f32.bin", "ref_f32_to_f6e3m2.bin", 64)
...
suite "E8M0 shared scale":
  ...
  test "decode all 256 vs ml_dtypes":
    ...
    if not fileExists(p):
      ...
      for c in 0 .. 255:
        ...
        let v = x.toFloat32
        if x.isNaN:
          if v == v: inc mm                          # 0xFF must decode to NaN
  ...
  test "encode f32 vs ml_dtypes (positive inputs; 0/inf/nan → 0xFF)":
    ...
    if not fileExists(p):
      ...
      for i in 0 ..< n:
        let want = s.readUint8()
        if bits(toE8M0(f32bits(ins[i]))) != want: inc mm
      check mm == 0
...
suite "MX packing":
  ...
  test "F4 pack/unpack round-trips (ragged)":
    for total in [16, 15]:
      ...
      for i in 0 ..< total: vals[i] = F4E2M1(uint8(i and 0x0f))
      ...
      var mm = 0
      for i in 0 ..< total:
        if uint8(un[i]) != uint8(i and 0x0f): inc mm
      check mm == 0
