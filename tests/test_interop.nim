...
suite "ml_dtypes / numpy names":
  ...
  test "unknown / deliberately-unimplemented names raise":
    expect ValueError:
      discard fromMlDtypesName("") # empty
    expect ValueError:
      discard fromMlDtypesName("float32") # not low-prec
    expect ValueError:
      discard fromMlDtypesName("float8_e3m4") # valid ml_dtypes, not implemented here
    expect ValueError:
      discard fromMlDtypesName("float8_e4m3") # plain variant != e4m3fn
    expect ValueError:
      discard fromMlDtypesName("uint4") # unsigned, no DType
...
suite "safetensors tags":
  ...
  test "valid safetensors tags with no low-prec DType raise":
    expect ValueError:
      discard fromSafetensorsName("") # empty
    expect ValueError:
      discard fromSafetensorsName("F32") # real tag, not low-prec
    expect ValueError:
      discard fromSafetensorsName("F64")
    expect ValueError:
      discard fromSafetensorsName("BOOL") # not a ±1 sign
    expect ValueError:
      discard fromSafetensorsName("U8") # unsigned, no DType
