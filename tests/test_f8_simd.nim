  ...
  suite nm:
    test "batch encode matches scalar over sweep + edges":
      ...
      for h in 0 ..< 65536:
        ...
        src.add v
        if abs(v) < 1e30: src.add v * 1.0000001'f32   # off-grid neighbour
      for v in [0.0'f32, -0.0, 448.0, 448.1, 463.99, 464.0, 464.001, 465.0,
                240.0, 247.9, 248.0, 248.1, 57344.0, 61439.0, 61440.0, 61441.0,
                1e-8, -1e-8, 6e-8, 1e30, -1e30, Inf, -Inf, NaN]:
        src.add v
      while src.len mod 8 != 3: src.add 1.0'f32       # ragged tail
      var got = newSeq[TT](src.len)
      batchFn(src, got)
      var mism = 0
      for i in 0 ..< src.len:
        if uint8(got[i]) != uint8(toFn(src[i])): inc mism
      check mism == 0
...
f8EncodeSuite(F8E4M3FNUZ, toF8E4M3FNUZ, toF8E4M3FNUZBatch, "fp8 e4m3fnuz batch ENCODE == scalar")
f8EncodeSuite(F8E5M2FNUZ, toF8E5M2FNUZ, toF8E5M2FNUZBatch, "fp8 e5m2fnuz batch ENCODE == scalar")
