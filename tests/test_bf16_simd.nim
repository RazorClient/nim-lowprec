...
suite "bf16 SIMD == scalar":
  ...
  test "toFloat32Batch matches scalar for all 65536 codes (aligned + ragged tail)":
    for total in [65536, 65533]:
      ...
      for i in 0 ..< total: src[i] = BF16(uint16(i and 0xffff))
      var simd = newSeq[float32](total)
      toFloat32Batch(src, simd)
      var mism = 0
      for i in 0 ..< total:
        if cast[uint32](simd[i]) != cast[uint32](toFloat32(src[i])): inc mism
      check mism == 0
  ...
  test "toBF16Batch matches scalar over a 100k fp32 sweep (ragged tail)":
    ...
    var mism = 0
    for i in 0 ..< src.len:
      if bits(simd[i]) != bits(toBF16(src[i])): inc mism
    check mism == 0
...
suite "fp16 SIMD == scalar (NaN-guarded)":
  ...
  test "f16 -> f32 for all 65536 codes":
    ...
    for i in 0 ..< 65536: src[i] = F16(uint16(i))
    ...
    for i in 0 ..< 65536:
      ...
      if s != s:                                   # scalar produced NaN
        if simd[i] == simd[i]: inc mism            # SIMD must also be NaN
      elif cast[uint32](simd[i]) != cast[uint32](s): inc mism
    check mism == 0
  ...
  test "f32 -> f16 over a 100k sweep":
    ...
    for i in 0 ..< src.len:
      if src[i] != src[i]:                         # NaN input
        if not simd[i].isNaN: inc mism
      elif bits(simd[i]) != bits(toF16(src[i])): inc mism
    check mism == 0
