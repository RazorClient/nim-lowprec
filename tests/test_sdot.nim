...
proc intRef(wq: openArray[I8]; wScales: openArray[float32]; groupSize: int;
...
proc packI4(vals: openArray[I4]): seq[byte] =
  ...
  for c in 0 ..< vals.len:
    let n = nibble(vals[c])
    if (c and 1) == 0: result[c shr 1] = n
    else:              result[c shr 1] = result[c shr 1] or (n shl 4)
...
suite "int8-activation GEMV == exact integer reference":
  echo "  dot-product path: ", (if lpUseDotProd: "SDOT (FEAT_DotProd)" else: "integer fallback")
  ...
  test "int8 weights, tiles + tile remainder + group tail":
    for (M, K, gs) in [(7, 96, 32), (4, 96, 96), (1, 96, 32), (9, 40, 20)]:
      ...
      for i in 0 ..< wq.len: wq[i] = I8(int8(max(nextByte(), -127)))
      for i in 0 ..< K: xq[i] = I8(int8(max(nextByte(), -127)))
      for i in 0 ..< wScales.len: wScales[i] = 0.011'f32 + float32(i mod 5) * 0.0013'f32
      for i in 0 ..< xScales.len: xScales[i] = 0.023'f32 + float32(i mod 3) * 0.0007'f32
  ...
  test "packed int4 weights, tiles + tile remainder + group tail":
    for (M, K, gs) in [(7, 128, 64), (4, 128, 128), (1, 128, 64), (5, 96, 48)]:
      ...
      template nextNib: int =
        u = u xor (u shl 13); u = u xor (u shr 7); u = u xor (u shl 17)
        int(u and 0x0f'u64) - 8
      for i in 0 ..< vals.len: vals[i] = toI4(float32(nextNib()))
      for i in 0 ..< K:
        u = u xor (u shl 13); u = u xor (u shr 7); u = u xor (u shl 17)
        xq[i] = I8(int8(max(int(u and 0xff'u64) - 128, -127)))   # keep it inside int8
      for i in 0 ..< wScales.len: wScales[i] = 0.017'f32 + float32(i mod 4) * 0.0011'f32
      for i in 0 ..< xScales.len: xScales[i] = 0.031'f32 + float32(i mod 3) * 0.0005'f32

      # The reference takes the same weights unpacked, one per byte.
      var asI8 = newSeq[I8](vals.len)
      for i in 0 ..< vals.len: asI8[i] = I8(int8(vals[i]))
      var want = newSeq[float32](M)
  ...
  test "packed and unpacked kernels agree on the same weights":
    ...
    const gs = 64
    var vals = newSeq[I4](M * K)
    for i in 0 ..< vals.len: vals[i] = toI4(float32((i mod 15) - 7))
    var asI8 = newSeq[I8](vals.len)
    for i in 0 ..< vals.len: asI8[i] = I8(int8(vals[i]))
    var xq = newSeq[I8](K)
    for i in 0 ..< K: xq[i] = I8(int8((i mod 200) - 100))
    var wScales = newSeq[float32](M * (K div gs))
    var xScales = newSeq[float32](K div gs)
    for i in 0 ..< wScales.len: wScales[i] = 0.02'f32
    for i in 0 ..< xScales.len: xScales[i] = 0.03'f32
...
suite "ggml-layout SDOT GEMV == exact reference":
  ...
  proc refQ8(w: openArray[BlockQ8_0]; bpr: int; xq: openArray[BlockQ8_0];
             y: var openArray[float32]) =
    for row in 0 ..< y.len:
  ...
  proc refQ4(w: openArray[BlockQ4_0]; bpr: int; xq: openArray[BlockQ8_0];
             y: var openArray[float32]) =
    for row in 0 ..< y.len:
      var lanes: array[4, float32]
      for bi in 0 ..< bpr:
  ...
  proc mkX(bpr: int): seq[BlockQ8_0] =
    ...
    for b in 0 ..< bpr:
      ...
      for i in 0 ..< QK: result[b].qs[i] = int8(((b * 29 + i * 13) mod 255) - 127)
  ...
  test "Q8_0 x q8 activations (4-chunks + block tail)":
    for (M, bpr) in [(5, 12), (4, 7), (1, 4), (3, 3)]:   # tails: bpr mod 4 = 0,3,0,3
      ...
      for b in 0 ..< w.len:
        ...
        for i in 0 ..< QK: w[b].qs[i] = int8(((b * 17 + i * 11) mod 255) - 127)
      let xq = mkX(bpr)
      var want = newSeq[float32](M)
      var got = newSeq[float32](M)
      refQ8(w, bpr, xq, want)
  ...
  test "Q4_0 x q8 activations (4-chunks + block tail)":
    for (M, bpr) in [(5, 12), (4, 7), (1, 4), (3, 3)]:
      ...
      for b in 0 ..< w.len:
        ...
        for i in 0 ..< QK div 2: w[b].qs[i] = uint8((b * 23 + i * 19) and 0xff)
      let xq = mkX(bpr)
      var want = newSeq[float32](M)
      var got = newSeq[float32](M)
      refQ4(w, bpr, xq, want)
...
suite "multi-column GEMM == per-column GEMV, bit-for-bit":
  ...
  test "int8 GEMM":
    for (M, K, gs, n) in [(9, 128, 64, 3), (4, 256, 128, 2), (5, 96, 48, 1), (6, 80, 40, 4)]:
      let gpr = K div gs
      var wq = newSeq[I8](M * K)
      var xq = newSeq[I8](n * K)
      var wScales = newSeq[float32](M * gpr)
      var xScales = newSeq[float32](n * gpr)
      for i in 0 ..< wq.len: wq[i] = I8(int8((i * 37 mod 251) - 125))
      for i in 0 ..< xq.len: xq[i] = I8(int8((i * 41 mod 241) - 120))
      for i in 0 ..< wScales.len: wScales[i] = 0.012'f32 + float32(i mod 6) * 0.0009'f32
      for i in 0 ..< xScales.len: xScales[i] = 0.021'f32 + float32(i mod 4) * 0.0013'f32
      ...
      for c in 0 ..< n:
        ...
        dequantGemvQ8(wq, wScales, gs,
                      xq.toOpenArray(c * K, (c + 1) * K - 1),
                      xScales.toOpenArray(c * gpr, (c + 1) * gpr - 1), yc)
        for r in 0 ..< M:
          if cast[uint32](yG[r * n + c]) != cast[uint32](yc[r]): inc mism
      check mism == 0

  test "packed int4 GEMM":
    for (M, K, gs, n) in [(9, 128, 64, 3), (4, 256, 128, 2), (5, 192, 96, 1), (6, 160, 80, 4)]:
      let gpr = K div gs
      var wp = newSeq[byte](M * (K div 2))
      var xq = newSeq[I8](n * K)
      var wScales = newSeq[float32](M * gpr)
      var xScales = newSeq[float32](n * gpr)
      for i in 0 ..< wp.len: wp[i] = uint8((i * 29 + 5) and 0xff)
      for i in 0 ..< xq.len: xq[i] = I8(int8((i * 43 mod 239) - 119))
      for i in 0 ..< wScales.len: wScales[i] = 0.014'f32 + float32(i mod 5) * 0.0011'f32
      for i in 0 ..< xScales.len: xScales[i] = 0.019'f32 + float32(i mod 3) * 0.0017'f32
