...
proc bitEq(a, b: openArray[float32]): int =
  for i in 0 ..< a.len:
    if cast[uint32](a[i]) != cast[uint32](b[i]):
      inc result
...
suite "SIMD dequant == scalar":
  ...
  test "int4, per-group (32)":
    ...
    for i in 0 ..< 512:
      x[i] = cos(float32(i) * 0.05'f32) * 3.0'f32
    let p = calibrateSymmetric(x, 32, 7.0'f32)
    var q = newSeq[I4](512)
    quantize(x, p, q)
    var a = newSeq[float32](512)
    dequantizeBatch(q, p, a)
    check bitEq(a, scalarDeq(q, p)) == 0
...
proc gemvRef(
  ...
  for row in 0 ..< M:
    ...
    for kk in 0 ..< K:
      ...
      acc +=
        float64(int8(wq[row * K + kk])) * float64(scales[row * gpr + g]) * float64(
          x[kk]
        )
    result[row] = acc
...
suite "SIMD fused dequant-GEMV":
  test "matches fp64 reference within tolerance (group tail exercised)":
    ...
    for i in 0 ..< wq.len:
      wq[i] = toI8(nextRnd(u) * 110.0'f32)
    for i in 0 ..< scales.len:
      scales[i] = 0.005'f32 + abs(nextRnd(u)) * 0.02'f32
    for i in 0 ..< K:
      x[i] = nextRnd(u) * 3.0'f32
...
suite "SIMD fused int4 GEMV (packed)":
  test "packed int4 GEMV matches fp64 reference within tolerance (tail exercised)":
    ...
    for r in 0 ..< M:
      for c in 0 ..< K:
        ...
        if (c and 1) == 0:
          ...
          wpacked[bi] = wpacked[bi] or (nib shl 4)
    for i in 0 ..< scales.len:
      scales[i] = 0.01'f32 + abs(nextRnd(u)) * 0.03'f32
    for i in 0 ..< K:
      x[i] = nextRnd(u) * 2.0'f32
    var y = newSeq[float32](M)
    ...
    for r in 0 ..< M:
      ...
      for c in 0 ..< K:
        acc +=
          float64(vals[r * K + c]) * float64(scales[r * gpr + c div gs]) * float64(x[c])
...
suite "SIMD MXFP4 fused GEMV (packed, E8M0 block scale)":
  test "packed MXFP4 GEMV matches fp64 reference within tolerance (tail exercised)":
    ...
    for r in 0 ..< M:
      ...
      for c in 0 ..< K:
        rowvals[c] = nextRnd(u) * 4.5'f32 # spread across the fp4 range
      let p = calibrateMX(rowvals, gs, 2) # MXFP4 emax = 2
      var qf4 = newSeq[F4E2M1](K)
      quantize(rowvals, p, qf4)
      for g in 0 ..< gpr:
        scales[r * gpr + g] = p.scale[g]
      var packed = newSeq[byte](rowBytes)
      packF4(qf4, packed)
      for bi in 0 ..< rowBytes:
        wpacked[r * rowBytes + bi] = packed[bi]
      var acc = 0.0'f64
      for c in 0 ..< K:
...
suite "ggml block dequant SIMD == scalar":
  ...
  test "Q8_0 batch dequant bit-identical":
    ...
    for b in 0 ..< NB:
      ...
      for i in 0 ..< QK:
        blocks[b].qs[i] = int8(((b * 31 + i * 7) mod 255) - 127)
  ...
  test "Q4_0 batch dequant bit-identical":
    ...
    for b in 0 ..< NB:
      ...
      for i in 0 ..< QK div 2:
        blocks[b].qs[i] = uint8((b * 13 + i * 17) and 0xff)
    var want = newSeq[float32](NB * QK)
    var got = newSeq[float32](NB * QK)
...
suite "ggml block quantize SIMD == scalar":
  ...
  test "quantizeQ8_0Batch bit-identical":
    ...
    for i in 5 * QK ..< 6 * QK:
      x[i] = 0.0'f32 # an all-zero block (d = 0 path)
...
suite "per-group I8 quantize SIMD == scalar":
  test "quantizeBatch bit-identical (per-group, per-tensor, ragged tail)":
    for (n, gs, scheme) in [
      (4096, 128, qPerGroup), (4099, 64, qPerGroup), (1000, 0, qPerTensor)
    ]:
      ...
      let p = (
        if scheme == qPerTensor: calibrateSymmetric(x, n, 127.0'f32)
