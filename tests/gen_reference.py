...
def emit_bf16():
    ...
    f32.view("<u4").tofile(os.path.join(REF_DIR, "ref_bf16_to_f32.bin"))
    ...
    with open(os.path.join(REF_DIR, "ref_f32_to_bf16.bin"), "wb") as fh:
...
def emit_f16():
    ...
    f32.view("<u4").tofile(os.path.join(REF_DIR, "ref_f16_to_f32.bin"))
    ...
    with open(os.path.join(REF_DIR, "ref_f32_to_f16.bin"), "wb") as fh:
        fh.write(np.uint32(len(ins)).tobytes())
        fh.write(ins.astype("<u4").tobytes())
        fh.write(outs.astype("<u2").tobytes())
...
def emit_fp8(name, dt):
    ...
    f32.view("<u4").tofile(os.path.join(REF_DIR, f"ref_{name}_to_f32.bin"))
    ...
    with open(os.path.join(REF_DIR, f"ref_f32_to_{name}.bin"), "wb") as fh:
        fh.write(np.uint32(len(ins)).tobytes())
        fh.write(ins.astype("<u4").tobytes())
...
def emit_e8m0():
    ...
    f32.view("<u4").tofile(os.path.join(REF_DIR, "ref_e8m0_to_f32.bin"))
    ...
    with open(os.path.join(REF_DIR, "ref_f32_to_e8m0.bin"), "wb") as fh:
        fh.write(np.uint32(len(ins)).tobytes())
        fh.write(ins.astype("<u4").tobytes())
        fh.write(outs.astype("<u1").tobytes())
...
def emit_ggml_schemes():
    ...
    x.view("<u4").tofile(os.path.join(REF_DIR, "ref_scheme_input.bin"))
    ...
    for name, t in [("q8_0", T.Q8_0), ("q4_0", T.Q4_0)]:
        ...
        b.tofile(os.path.join(REF_DIR, f"ref_ggml_{name}_bytes.bin"))
        d.view("<u4").tofile(os.path.join(REF_DIR, f"ref_ggml_{name}_dq.bin"))
    ...
    for name, t, blocks in [("q4_k", T.Q4_K, q4k), ("q6_k", T.Q6_K, q6k)]:
        ...
        raw.tofile(os.path.join(REF_DIR, f"ref_ggml_{name}_bytes.bin"))
        d.view("<u4").tofile(os.path.join(REF_DIR, f"ref_ggml_{name}_dq.bin"))
...
def emit_mx_schemes():
    ...
    x = x.reshape(-1)
    x.view("<u4").tofile(os.path.join(REF_DIR, "ref_mx_input.bin"))
    ...
    for name, dt, emax in formats:
        ...
        dq.reshape(-1).view("<u4").tofile(os.path.join(REF_DIR, f"ref_mx_{name}_dq.bin"))
...
def emit_nvfp4():
    ...
    xf = x.reshape(-1)
    xf.view("<u4").tofile(os.path.join(REF_DIR, "ref_nvfp4_input.bin"))
    ...
    with open(os.path.join(REF_DIR, "ref_nvfp4_out.bin"), "wb") as fh:
...
if __name__ == "__main__":
    ...
    emit_mx_schemes()
    emit_nvfp4()
    print(f"wrote element + block-scheme reference vectors to {REF_DIR}")
