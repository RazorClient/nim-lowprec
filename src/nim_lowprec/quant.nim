...
func absMaxScale(x: openArray[float32]; lo, hi: int; qmax: float32): float32 {.inline.} =
  ...
  if amax == 0.0'f32: 1.0'f32 else: amax / qmax
...
func mxSharedScale(amax: float32; elemEmax: int): float32 {.inline.} =
  ...
  if amax == 0.0'f32: return 1.0'f32
...
func affineParams(x: openArray[float32]; lo, hi: int; qmin, qmax: float32): (float32, float32) =
...
proc calibrateAsymmetric*(x: openArray[float32]; groupSize: int; qmin, qmax: float32): QParams =
