...
when defined(arm64) or defined(aarch64):
  ...
  proc vcvtaq_s32_f32*(a: float32x4): int32x4 {.importc.}
    # round ties-AWAY -> int32 (matches Nim round())
  ...
  proc vsubq_s8*(a, b: int8x16): int8x16 {.importc.}
    # per-lane int8 subtract (ggml Q4_0 nibble-8)
  ...
  proc vaddq_f32*(a, b: float32x4): float32x4 {.importc.}
  ...
  proc vshlq_u8*(a: uint8x16, b: int8x16): uint8x16 {.importc.}
    # var shift (b<0 ⇒ right)
  ...
  proc vshlq_u16*(a: uint16x8, b: int16x8): uint16x8 {.importc.}
    # var shift (b<0 ⇒ right)
