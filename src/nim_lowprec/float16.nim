...
func halfbitsFromFloat(f: uint32): uint16 =
  ...
  if f_exp <= 0x3800_0000'u32:
    ...
    if ((f_sig and 0x0000_3fff'u32) != 0x0000_1000'u32) or
        ((f and 0x0000_07ff'u32) != 0'u32):
...
func toF16*(x: float64): F16 {.inline.} =
  toF16(x.float32)

func isNaN*(x: F16): bool {.inline.} =
  (uint16(x) and 0x7fff'u16) > 0x7c00'u16
func isInf*(x: F16): bool {.inline.} =
  (uint16(x) and 0x7fff'u16) == 0x7c00'u16
func signbit*(x: F16): bool {.inline.} =
