...
proc dequantizeQ6_K*(blocks: openArray[BlockQ6_K]; dst: var openArray[float32]) =
  ...
  for bi in 0 ..< blocks.len:
    ...
    while n < QK_K:
      for l in 0 ..< 32:
        let isv = l div 16
        let q1 = int(int8((blocks[bi].ql[ql + l]      and 0x0f'u8) or (((blocks[bi].qh[qh + l] shr 0) and 3) shl 4))) - 32
        let q2 = int(int8((blocks[bi].ql[ql + l + 32] and 0x0f'u8) or (((blocks[bi].qh[qh + l] shr 2) and 3) shl 4))) - 32
        let q3 = int(int8((blocks[bi].ql[ql + l]      shr 4)       or (((blocks[bi].qh[qh + l] shr 4) and 3) shl 4))) - 32
        let q4 = int(int8((blocks[bi].ql[ql + l + 32] shr 4)       or (((blocks[bi].qh[qh + l] shr 6) and 3) shl 4))) - 32
